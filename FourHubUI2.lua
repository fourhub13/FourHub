...
local FourHublib = {
    Themes = {
        QuangHuy = {
            ...
            ["Color Toggle On"] = Color3.fromRGB(0, 191, 255),
            ["Color Toggle Off"] = Color3.fromRGB(0, 0, 0),
            ["Color Toggle Knob On"] = Color3.fromRGB(255, 255, 255),
            ["Color Toggle Knob Off"] = Color3.fromRGB(0, 191, 255),
...
local Settings = FourHublib.Settings
local Flags = FourHublib.Flags

local SetProps, SetChildren, InsertTheme, Create do
    InsertTheme = function(Instance, Type)
        table.insert(FourHublib.Instances, {
            Instance = Instance,
            Type = Type
        })
        return Instance
    ...
    local function Save(file)
        if readfile and isfile and isfile(file) then
            ...
            if type(decode) == "table" then
                if rawget(decode, "UISize") then FourHublib.Save["UISize"] = decode["UISize"] end
                if rawget(decode, "TabSize") then FourHublib.Save["TabSize"] = decode["TabSize"] end
                if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then FourHublib.Save["Theme"] = decode["Theme"] end
            end
        end
    end
    
    pcall(Save, "FourHub library v8.1.json")
end
...
local Connections, Connection = {}, FourHublib.Connection do
...
local function VerifyTheme(Theme)
    for name,_ in pairs(FourHublib.Themes) do
        if name == Theme then
            return true
        end
    end
...
local Theme = FourHublib.Themes[FourHublib.Save.Theme]

local function AddEle(Name, Func)
    FourHublib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
    local Element = FourHublib.Elements[Ele](Instance, props, ...)
    return Element
end
...
function FourHublib:GetIcon(index)
...
function FourHublib:SetTheme(NewTheme)
    if not VerifyTheme(NewTheme) then return end
    
    FourHublib.Save.Theme = NewTheme
    SaveJson("FourHub library v8.1.json", FourHublib.Save)
    
    local OldTheme = Theme
    Theme = FourHublib.Themes[NewTheme]
    ...
    for _, Val in pairs(FourHublib.Instances) do
    ...
    for _, TabData in pairs(FourHublib.Tabs) do
...
function FourHublib:SetScale(NewScale)
...
function FourHublib:MakeWindow(Configs)
    local WTitle = Configs[1] or Configs.Name or Configs.Title or "FourHub Library v8.1"
    local WMiniText = Configs[2] or Configs.SubTitle or "by : jokerbiel13"
    ...
    local UISizeX, UISizeY = unpack(FourHublib.Save.UISize)
    ...
    MinimizedIcon = Create("ImageLabel", MinimizedContainer, {
        ...
        AnchorPoint = Vector2.new(0, 0.5),
        BackgroundTransparency = 1,
        Image = "rbxassetid://123035506495251",
    ...
    local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
        Size = UDim2.new(0, FourHublib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
    ...
    ConnectSave(ControlSize1, function()
        if not Minimized then
            FourHublib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
            SaveJson("FourHub library v8.1.json", FourHublib.Save)
        end
    end)
    
    ConnectSave(ControlSize2, function()
        FourHublib.Save.TabSize = MainScroll.Size.X.Offset
        SaveJson("FourHub library v8.1.json", FourHublib.Save)
    end)
    ...
    local function UpdateSearch(Query)
        ...
        for _, ElementData in pairs(FourHublib.AllElements) do
    ...
    function Window:SelectTab(TabSelect)
        if type(TabSelect) == "number" then
            FourHublib.Tabs[TabSelect].func:Enable()
        else
            for _,Tab in pairs(FourHublib.Tabs) do
    ...
    function Window:MakeTab(paste, Configs)
        ...
        TIcon = FourHublib:GetIcon(TIcon)
        ...
        local function Tabs()
            ...
            table.foreach(FourHublib.Tabs, function(_,Tab)
        ...
        FirstTab = true
        local Tab = {}
        table.insert(FourHublib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
        Tab.Cont = Container
        ...
        function Tab:AddSection(Configs)
            ...
            table.insert(FourHublib.AllElements, {
                Name = SectionName,
            ...
            local Section = {}
            table.insert(FourHublib.Options, {type = "Section", Name = SectionName, func = Section})
    ...
    task.spawn(function()
        ...
        ToggleGui.Name = "FourHubHub_Toggle_Circle"
        ...
        ToggleButton.Image = "rbxassetid://123035506495251"
...
local function CreateNotification(Icon, Title, Message, Duration)
    ...
    IconImage.Image = Icon or "rbxassetid://123035506495251"
    ...
    TitleLabel.Text = Title or "FourHub Hub"
...
function FourHublib:Notify(Configs)
    local Title = Configs.Title or Configs[1] or "FourHub Hub"
    local Message = Configs.Message or Configs[2] or Configs.Text or ""
    local Icon = Configs.Icon or "rbxassetid://123035506495251"
...
function FourHublib:SetUIBorderColor(color)
    if typeof(color) == "Color3" then
        self.Themes[self.Save.Theme]["UI Border Color"] = color
        if self.Save.Theme == FourHublib.Save.Theme then
...
function FourHublib:SetDiscordBorderColor(color)
    if typeof(color) == "Color3" then
...
function FourHublib:SetProfileBorderColor(color)
    if typeof(color) == "Color3" then
...
function FourHublib:SetHubColor(color)
    if typeof(color) == "Color3" then
...
function FourHublib:SetStrokeColor(color)
    if typeof(color) == "Color3" then
...
function FourHublib:SetTextColor(color)
    if typeof(color) == "Color3" then
...
function FourHublib:SetToggleOnColor(color)
    if typeof(color) == "Color3" then
...
function FourHublib:SetToggleOffColor(color)
    if typeof(color) == "Color3" then
...
function FourHublib:SetToggleKnobOnColor(color)
    if typeof(color) == "Color3" then
...
function FourHublib:SetToggleKnobOffColor(color)
    if typeof(color) == "Color3" then
...
function FourHublib:SetToggleBorderColor(color)
    if typeof(color) == "Color3" then
...
function FourHublib:SetBorderThickness(thickness)
...
function FourHublib:GetBorderColors()
    return {
...
function FourHublib:GetToggleColors()
    return {
...
function FourHublib:ResetBorderColors()
...
function FourHublib:ResetToggleColors()
    local defaultTheme = self.Themes.QuangHuy
...
task.spawn(function()
    task.wait(2)
    FourHublib:Notify({
        Title = "FourHub Library",
        Message = "UI FourHub Library Download Completed",
        Duration = 10
    })
end)
